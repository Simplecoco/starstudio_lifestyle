...
var songStatus = "stop";
var nowSong = "锦鲤抄";
//这里要是这个样子啊，两段时间之间要有歌词或者空格，还有LRC文件前面[]里还会有不是这种表示时间的东西，删掉吧。。。还有开头和结尾都要是[xxxxx]
	...
	for(let i = 0; i < bns.length; i++){
		...
		bns[i].onclick = function(){
			...
			for(let j = 0; j < songs.length; j++){
				console.log(songs[j].title == name)
...
function play(){
	...
	songPic.innerHTML = "<img src='img/" + songs[seekNum()].title + ".jpg'/>"
...
function lyricsSynchronization(name){                  //也可以用this的吧，调那个歌词对象。
	...
	//console.log(setTimeout(listener,500));
	...
	function listener(){
		...
		if(nowPosition!=lyricsCount && oldSong==nowSong) {
			setTimeout(listener, 500);
		}
	}
	...
	//		if (audio.currentTime >= formattingLyrics[i]["time"]) {
	//			lyricsDom.innerText = formattingLyrics[i].lyric;
	//			//var timeGap=(formattingLyrics[i + 1]["time"] - formattingLyrics[i]["time"])*1000;
	//		}
	//	}
	//	if(nowPosition!=lyricsCount) {
	//		setTimeout(listener, 500);
	//	}
	//}
	...
	//	//var i=0;
	//			//if (audio.currentTime >= formattingLyrics[i]["time"]) {
	//	//for(var i=0;i<=lyricsCount;i++){
	//				lyricsDom.innerText = formattingLyrics[nowPosition].lyric;
	//				var timeGap=(formattingLyrics[nowPosition + 1]["time"] - formattingLyrics[nowPosition]["time"])*1000;
	//				console.log(timeGap);
	//			//}
	//	//}
	//	nowPosition++;
	//	if(nowPosition!=lyricsCount-1) {
	//		setTimeout(listener, timeGap);
	//	}
	//}
